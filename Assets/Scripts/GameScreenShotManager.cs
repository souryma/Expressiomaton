...
public class GameScreenShotManager : MonoBehaviour
{
    ...
    private string PathCreator(bool p_IsGameScreenShot = false)
    {
        string filename;

        if (p_IsGameScreenShot)
        {
            filename = "Shot-o-maton Game " + DateTime.Now.ToString("yyyy-MM-ddTHH-mm-ss-f") + ".png";
        }
        else
        { 
            filename = "Shot-o-maton " + DateTime.Now.ToString("yyyy-MM-ddTHH-mm-ss-f") + ".png";
